...
function WsConnector (id, opts) {
  ...
  if (port != null || server != null) {
    if (port != null) {
      ...
      if (port !== 0) self.url = urlFormat(port)
    } else {
      self._wss = new WebSocket.Server({ server: server })
      self.url = urlFormat(server.address().port)
    }
    self._wss.on('connection', onConnection)
    self._wss.on('listening', onListen)
  }

  function urlFormat (port) {
  ...
  function onListen () {
    ...
    self.url = urlFormat(self._wss._server.address().port)
  }
}
