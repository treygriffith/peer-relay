...
function Router (channels, id) {
  ...
  self._channels.toArray().forEach(function (c) {
...
Router.prototype._send = function (msg) {
  ...
  var closests = self._channels.closest(target, 20)
    .filter(function (c) { return msg.path.indexOf(c.id.toString('hex')) === -1 })
    .filter(function (_, index) { return index < self.concurrency })
  ...
  for (var i = 0; i < closests.length; i++) {
    ...
    closests[i].send(msg)
    if (closests[i].id.toString('hex') === msg.to) break
  }
}
