...
function Client (opts) {
  ...
  self.id = opts.id || crypto.randomBytes(20)
  if (!Buffer.isBuffer(self.id)) {
    throw new Error('Id must be a Buffer')
  }
  if (self.id.length !== 20) {
    throw new Error('Id must be 160 bits')
  }
  self.pending = {}
  ...
  self.wsConnector = new WsConnector(self.id, {
    host: opts.host,
    port: opts.port,
    server: opts.server,
    secure: opts.secure
  ...
  self._debug('Client(%s)', JSON.stringify(opts, ['host', 'port', 'bootstrap']))

  var bootstrap = opts.bootstrap || []
  bootstrap.forEach(function (uri) {
...
Client.prototype._onFindPeers = function (msg, from) {
  ...
  self.router.send(from, {
    type: 'foundPeers',
    data: closest.map(function (e) { return e.id.toString('hex') })
  })
}

Client.prototype._onFoundPeers = function (msg) {
  var self = this
  msg.data.forEach(function (canidate) {
    self.canidates.add({
